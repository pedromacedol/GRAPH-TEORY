...
class MeuGrafo(GrafoListaAdjacencia):
    ...
    def caminho(self, n):
        ...
        '''
        grafo_caminho = copy.deepcopy(self)
        grafo_caminho.caminho_remover_paralelas()
        caminho = []
        aresta_visitadas = []
        vertice = ''
        for i in range(n):
    ...
    def caminho_remover_paralelas(grafo):
        '''
        ...
        if grafo_novo.ha_paralelas()== True:
            lista_paralelas = []
            lista = []
            for vertice in grafo.N:
                for aresta in grafo.A:
                    par_vertice = [vertice, grafo.A[aresta].getV2()]
                    if vertice == grafo.A[aresta].getV1():
                        if lista == []:
                            lista.append(par_vertice)
                        elif par_vertice in lista or list(reversed(par_vertice)) in lista:
                            lista.append(par_vertice)
    ...
    def caminho_euleriano(self):
        if self.conexo() == True:
            ...
            if len(vertices) == 2 or vertices == []:
                return self.fleurys_algorithm(vertices)
            else:
                return False

    def fleurys_algorithm (self, vertices):
        ...
        prox_v = v0
        while list(grafo.A) != []:
            for a in grafo.arestas_sobre_vertice(prox_v):
                vertice_1 = grafo.A[a].getV1()
                vertice_2 = grafo.A[a].getV2()
                
                if lista == []:
                    if vertice_1 == v0 and vertice_2 != vf or vertice_2 == v0 and vertice_1 != vf or grafo.grau(prox_v) == 1:
                        lista.append(v0) 
                        lista.append(a)
                        if vertice_1 == v0:
                ...
                elif vertice_1 == prox_v or vertice_2 == prox_v:
                    lista.append(a)
                    if vertice_1 == prox_v:
                        lista.append(vertice_2) 
                        prox_v = vertice_2
                    elif vertice_2 == prox_v:
                        lista.append(vertice_1)
                        prox_v = vertice_1
                    grafo.removeAresta(a)
